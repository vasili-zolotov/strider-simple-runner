...
Job.prototype = {
  ...
  forkProc: function (cwd, command, args, next) {
    var env = _.extend({}, process.env)
      , screencmd = null
      , proc;
    env.PAAS_NAME = 'strider'; // TODO: define additional dynamic environment variables here

    // repo_ssh_url looks something like this: https://github.com/vasili-zolotov/strider-paperg-worker.git
    // export domain name for this github repo: branch.user.project
    var url_tokens = this.data.repo_ssh_url.replace('.git','').replace('git@github.com:','').split('/');
    ...
    env.STRIDER_REPO_DOMAIN = [branch, user, repo].join('.');
    
    // export job id, so tests can link back to strider job
  ...
  gitStep: function (done, cb) {
    ...
    if (!fs.existsSync(self.dir + '/.git') || !self.config.gitUpdate) {
      return cmd.run({
        ...
        var command = 'git clone --recursive ' + urls[0] + ' .; git checkout ' + branch;
        cmd.git({
          io: self.io,
          cwd: self.dir,
    ...
    Step(
      ...
      function (code) {
        ...
        cmd.run({
          io: self.io,
          cwd: self.dir,
          command: 'git fetch origin; git checkout ' + branch,
          pty: self.config.pty // should we just never to pty here?
        }, this)
      },
      function (code) {
