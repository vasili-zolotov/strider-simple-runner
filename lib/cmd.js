...
// default timeout: 40m, override globally using COMMAND_TIMEOUT env var
var COMMAND_TIMEOUT = process.env.COMMAND_TIMEOUT || '2400';
...
// This is expected to be bound to a Job object.
function shellWrap(cmd) {
  // try to grab timeout override from project properties
  // make sure we end up with a string
  var timeout = String(
    Number(this.data && this.data.repo_config && this.data.repo_config.env && this.data.repo_config.env.COMMAND_TIMEOUT)
    || COMMAND_TIMEOUT
  );
  return {
    cmd: 'timeout',
    args: [COMMAND_TIMEOUT, 'bash', '-c', cmd]
  }
}

function shellUnwrap(cmd, args) {
  if (cmd === 'timeout' && args.length === 4 && args[2] === '-c') {
    cmd = args[3]
  } else {
    cmd += ' ' + args.join(' ')
  }
...
function run(io, cwd, command, next) {
  ...
  if (!options.args) {
    options.args = [COMMAND_TIMEOUT, 'bash', '-c', options.command];
