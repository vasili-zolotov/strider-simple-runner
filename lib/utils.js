...
module.exports = {
  ...
  domainForGithubRepoBranch: domainForGithubRepoBranch,
...
// Generate a domain name for a given branch of a given github repo.
// repo_ssh_url looks something like this: git@github.com:vasili-zolotov/strider-simple-runner.git
// Domain name for the master branch of the above repo is: master.vasili-zolotov.strider-paperg-worker
function domainForGithubRepoBranch(repo_ssh_url, branch) {
  if (!repo_ssh_url) throw new Error('repo_ssh_url is empty');
  if (!branch) throw new Error('branch is empty');
  if (typeof repo_ssh_url != 'string') throw new Error('repo_ssh_url is not a string');
  if (typeof branch != 'string') throw new Error('branch is not a string');
  if (!/^git\@github\.com\:([^\/]+)\/(.+)\.git$/.test(repo_ssh_url)) throw new Error('repo_ssh_url is not correct');
  
  var url_tokens = repo_ssh_url.replace('.git','').replace('git@github.com:','').split('/');
  var repo = url_tokens[1];
  var user = url_tokens[0];
  var domain = [branch.replace(/\./g, '-').replace(/^([0-9\-]+)/,'v$1'), user, repo].join('.');
  
  return normalizeDomain(domain);
}

// Normalize domain name (per 2.3.1 of RFC-1035, https://tools.ietf.org/html/rfc1035):
// For each label:
//  convert to lowercase (important for string comparisons to work correctly)
//  replace anything other than letter, digit or hyphen with '-'
//  remove leading digits, and '-' that are leading or trailing
function normalizeDomain(domain) {
  if (!domain) throw new Error("domain is empty");
  if (typeof domain != 'string') throw new Error("domain is not a string");
  
  domain = domain.split('.').map(function(label){ 
