...
AWS.config.update({ accessKeyId: process.env.ENV_VARIABLE, secretAccessKey: process.env.ENV_VARIABLE });
...
Worker.prototype = {
  attach: function (emitter) {
    ...
    emitter.on('queue.new_job', function (task) {
      ...
      self.debug('Enqueue job: domain = %s, id = %s', domain, task.job_id);  // e.g. master.placelocal.paperg
  ...
  processJob: function (data, next) {
    ...
    self.debug('processJob: job_id = %s', data.job_id)
    
    d.run(function () {
      var shortname = data.repo_config.url.split('/').slice(-2).join('/')
        , dirname = data.user_id + '.' + shortname.replace('/', '.')
        , branch = data.github_commit_info && data.github_commit_info.branch || "master" // TODO: assume master branch if github_commit_info is absent (manual start)
